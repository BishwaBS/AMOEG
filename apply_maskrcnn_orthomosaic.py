...
def define_predictor(modeldir):
    ...
    #loading the weights
    cfg.OUTPUT_DIR = os.path.join(os.getcwd(), modeldir)

    modelfile=glob.glob(cfg.OUTPUT_DIR + "/*.pth")
    cfg.MODEL.WEIGHTS = modelfile[0]
...
def apply_model_mosaic(raster_dir, partition_number, modeldir, outputdir):
    predictor=define_predictor(modeldir)
    ...
    for raster in glob.glob(os.path.join(os.getcwd(), raster_dir) +  "/*.tif"): #iterating over all the tif files found in the raster directory       
          ...
          print("...{} objects were detected in {}...".format(gdf.shape[0], plotname))
...
def main():
    ...
    parser.add_argument("modeldir", help="Relative location of dir that contains trained model file", type=str)
    ...
    apply_model_mosaic(args.raster_dir, args.partition_number, args.modeldir, args.outputdir)      
