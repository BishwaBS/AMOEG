...
class COCOEvaluator(DatasetEvaluator):
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        for idx, name in enumerate(class_names):
            ...
            precision05=precisions[0,:,idx, 0, -1]
            val=np.round(np.mean(precision05),2)
            cls_precision[name]=val
            


            ap = np.mean(precision) if precision.size else float("nan")
            # results_per_category.append(("{}".format(name), precision))
        ...
        print(cls_precision)
        a=cls_precision["MG"]
        b=cls_precision["Grass"]
        avg=(a+b)/2
        print("mAP@05 is {}".format(avg))
